...
local cpu = lain.widget.cpu({
    settings = function()
        local core_usage = cpu_now[1].usage .. "%"
        for i = 2, 4 do
            core_usage = core_usage .. " " .. cpu_now[i].usage .. "%"
        end
        widget:set_markup(markup(theme.color1,
                                 "⚙ " .. cpu_now.usage .. "% (" .. core_usage ..
                                     ")"))
    end
})
...
local mem = lain.widget.mem({
    settings = function()
        widget:set_markup(markup(theme.color2, "🐏 " ..
                                     math.floor((mem_now.used / 10.24) + 0.5) /
                                     100 .. "GB"))
    end
})
...
local temp = lain.widget.temp({
    settings = function()
        widget:set_markup(
            markup(theme.color3, "🌡 " .. coretemp_now .. "°C"))
    end
})
...
local bat = lain.widget.bat({
    settings = function()
        ...
        widget:set_markup(markup(theme.color4, icon .. "  " .. perc))
    end
})
...
local volume = lain.widget.pulse({
    settings = function()
        ...
        if tonumber(volume_now.left) >= 50 then
          icon = "🔊"
        end
        ...
        widget:set_markup(
            markup(theme.color5, icon .. text))
    end
})

local weather = lain.widget.weather({
    city_id = 2964574,
    weather_na_markup = markup(theme.color7, "N/A"),
    settings = function()
        descr = weather_now["weather"][1]["description"]
        units = math.floor(weather_now["main"]["temp"])
        widget:set_markup(markup(theme.color7,
                                 "☀  " .. units .. "°C  —  " .. descr))
    end
})
...
local net = lain.widget.net({
    ...
    settings = function()
        ...
        -- local signal = require "pl.pretty".write(net_now.devices, "")
        local signal = "No signal"
        local wlp2s0 = net_now.devices.wlp2s0
        if wlp2s0 then
          if wlp2s0.wifi then
            signal = wlp2s0.signal .. " dBm"
          end
        end
        widget:set_markup(markup(theme.color6, "📶 " .. signal .. "  —  ⬇" .. net_now.received .. "KB  —  ⬆" ..
                                     net_now.sent .. "KB"))
    end
})

local textclock = wibox.widget.textclock(
                      markup(theme.color8, "%A, %d %B  —  %X"), 2)
    ...
    }, -- Floating clients.
    {
        rule_any = {
            ...
            class = {
                ...
                "Pavucontrol", "Sxiv", "Tor Browser", -- Needs a fixed window size to avoid fingerprinting by screen size.
                "Wpa_gui", "veromix", "xtightvncviewer", "mpv", "feh"
            },
